...
@section('content')
    ...
    <form action="{{ isset($articles) ? route('articles.update', $articles->id) : route('articles.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        @if(isset($articles))
            @method('PUT')  
        @endif
        ...
        <div class="form-group">
            ...
            @if(@$articles)
                <div class="mt-2">
                    <img src="{{ asset('storage/articles/' . basename($articles->image)) }}" class="img-fluid rounded" alt="Article Image" width="100">
                </div>
            @endif
            @error('error')
                <div class="invalid-feedback">{{ $message }}</div>
        ...
        <a href="{{ isset($articles) && $articles->status == 'published' ? route('articles.index') : route('drafts.index') }}" class="btn btn-danger btn-sm mt-3">Cancel</a>
    </form>
